    ...
    <div class="flex items-center gap-x-9 max-[1100px]:gap-x-6 max-lg:gap-x-8">
        ...
        <div class="mt-1.5 flex gap-x-8 max-[1100px]:gap-x-6 max-lg:gap-x-8">
            ...
            @if (core()->getConfigData('customer.settings.wishlist.wishlist_option'))
                <a href="{{ route('shop.customers.account.wishlist.index') }}" aria-label="Wishlist" class="relative">
...
@pushOnce('scripts')
    ...
    <script>
    (function() {
        const countsUrl = "{{ route('wishlistcompare.counts') }}";
        ...
        async function refreshCounts() {
            try {
                ...
                if (compareItems.length) {
                    params.set('compare_items', JSON.stringify(compareItems));
                }

                const url = params.toString() ? `${countsUrl}?${params}` : countsUrl;
                ...
                if (wishBadge) {
                    if (typeof wishlist === 'number' && wishlist > 0) {
                        wishBadge.textContent = wishlist;
                ...
                if (compBadge) {
                    if (typeof compare === 'number' && compare > 0) {
                        compBadge.textContent = compare;
